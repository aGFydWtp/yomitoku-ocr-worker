...
- [ ] 7.1. StatusTable の PK を `job_id` に変更する（P0）
  - [ ] 7.1.1. `lib/processing-stack.ts` の StatusTable 定義を変更
  ...
  - [ ] 7.1.2. `test/processing-stack.test.ts` を更新し、PK が `job_id` であること・GSI 構成が正しいことを検証
  - [ ] 7.1.3. `npx cdk synth` でテンプレート生成が成功することを確認

- [ ] 7.2. processor Lambda を `job_id` ベースに更新する（P0、7.1 完了後）
  - [ ] 7.2.1. `lambda/processor/tests/` のテストを先に更新する（TDD）
  ...
  - [ ] 7.2.2. `lambda/processor/index.py` を更新
  ...
  - [ ] 7.2.3. `pytest lambda/processor/tests/ -v` で全テストが通ることを確認

- [ ] 7.3. processor Lambda に PDF マジックナンバー検証を追加する（P1）
  - [ ] 7.3.1. テストを先に作成する（TDD）
    - 正常 PDF（先頭 `%PDF-`）→ 処理続行
    - 不正ファイル（先頭が `%PDF-` でない）→ `ValueError` を送出し FAILED に遷移
  - [ ] 7.3.2. `lambda/processor/index.py` の S3 ダウンロード後・OCR 処理前に検証を追加
    ```python
    ...
    ```
  - [ ] 7.3.3. `pytest lambda/processor/tests/ -v` で全テストが通ることを確認

- [ ] 7.4. デプロイして既存パイプラインの動作を確認する（P0、7.2 完了後）
  - [ ] 7.4.1. StatusTable の `removalPolicy` を一時的に `DESTROY` に変更してデプロイ（テーブル再作成）
  - [ ] 7.4.2. デプロイ後に `removalPolicy` を `RETAIN` に戻してデプロイ
  - [ ] 7.4.3. S3 キー `input/{uuid}/test.pdf` 形式で PDF をアップロードし、processor が `job_id` で DynamoDB を更新することを確認
  - [ ] 7.4.4. 冪等性テスト（`scripts/test-idempotency.py`）を `job_id` 対応に更新して実行
