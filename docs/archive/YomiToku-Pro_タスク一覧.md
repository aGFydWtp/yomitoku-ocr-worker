...
- [x] 8.1. `lambda/api/` ディレクトリをセットアップする（P0）
  - [x] 8.1.1. ディレクトリ構成を作成
    ```
    lambda/api/
      index.ts
      ...
      __tests__/
        ...
        lib/
          sanitize.test.ts
    ```
  - [x] 8.1.2. `lambda/api/package.json` を作成
  ...
  - [x] 8.1.3. `lambda/api/tsconfig.json` を作成（`target: ES2022`, `module: ESNext`, `moduleResolution: bundler`）
  - [x] 8.1.4. `pnpm install` で依存関係をインストールし、`npx tsc --noEmit` がエラーなしで通ることを確認

- [x] 8.2. 共通ライブラリを実装する（P0、8.1 完了後）
  - [x] 8.2.1. ファイル名サニタイズのテストを作成する（TDD: `__tests__/lib/sanitize.test.ts`）
    - 正常系: `"請求書.pdf"` → `"請求書.pdf"`
    ...
    - Unicode パス区切り文字 U+2215: `"path∕to∕file.pdf"` → `"file.pdf"`
    - ファイル名長制限: 255 バイト超 → `ValidationError`
    - マルチバイト文字の長さ検証: UTF-8 バイト数で 255 バイト超 → `ValidationError`
    - 全 15 テスト PASS
  - [x] 8.2.2. `lib/sanitize.ts` を実装してテストを通す
    - `sanitizeFilename(raw: string): string` — ディレクトリ除去、制御文字除去、`.pdf` 検証、Unicode パス区切り除去、255 バイト長制限
  - [x] 8.2.3. `lib/dynamodb.ts` を実装（`DynamoDBDocumentClient` をハンドラ外で初期化、export）
  - [x] 8.2.4. `lib/s3.ts` を実装
  ...
  - [x] 8.2.5. `lib/errors.ts` を実装（`ValidationError`, `NotFoundError`, `ConflictError` クラス + Hono `onError` ハンドラ、`err: unknown` 型対応）

- [x] 8.3. `POST /jobs` のテストと実装（TDD）（P0、8.2 完了後）
  - [x] 8.3.1. テストを作成する（`__tests__/routes/jobs.test.ts`）
    ...
    - 正常系: S3 Presigned URL が正しいバケットとキーで発行される
    - 正常系: 日本語ファイル名が正しく処理される
    - バリデーション: `filename` 未指定 → 400
    - バリデーション: `filename` が `.pdf` でない → 400
    - バリデーション: `filename` が空文字 → `"document.pdf"` にフォールバックし 201
    - 全 7 テスト PASS
  - [x] 8.3.2. `routes/jobs.ts` に `POST /` ルートを実装してテストを通す
    ...
    - 201 レスポンス返却
    - 環境変数はハンドラ内で読み取り（テスト時の `beforeEach` での設定に対応）

- [x] 8.4. `GET /jobs/:jobId` のテストと実装（TDD）（P0、8.2 完了後）
  - [x] 8.4.1. テストを作成する（10テストケース）
    ...
    - 正常系: DynamoDB GetItem が強整合性読み取り（ConsistentRead: true）で呼ばれる
    - 異常系: 存在しない jobId → 404
    - 異常系: jobId が UUID 形式でない → 400（DynamoDB 呼び出しなし）
    - 異常系: COMPLETED 状態で S3 Presigned URL 生成失敗 → 500
    - 異常系: DynamoDB 読み取り失敗 → 500
  - [x] 8.4.2. `routes/jobs.ts` に `GET /:jobId` ルートを実装してテストを通す
    - UUID 形式バリデーション（小文字のみ許可）
    - `GetCommand`（PK: `job_id`、`ConsistentRead: true`）
    - COMPLETED の場合は `createResultUrl()` で Presigned GET URL を発行
    - FAILED の場合は `error_message` を含める
    - 未検出は 404（`NotFoundError`）
    - レスポンスに型付きインターフェース `JobResponse` を使用

- [x] 8.5. `GET /jobs` のテストと実装（TDD）（P0、8.2 完了後）
  - [x] 8.5.1. テストを作成する（14テストケース）
    - 正常系: `?status=COMPLETED` → 200、`items` 配列、`count`、`cursor`
    - 正常系: `?status=PENDING&limit=5` → 最大 5 件
    - ページネーション: `cursor` を使って次ページ取得 → 正しい続きが返る
    - ページネーション: `cursor` を渡して次ページを取得できる（ExclusiveStartKey 検証）
    - 最終ページ: `cursor: null`
    - 一覧に `resultUrl` が含まれないことを確認
    - GSI `status-created_at-index` で Query が呼ばれる検証
    - バリデーション: `status` 未指定 → 400
    - バリデーション: `limit` が 0 以下 → 400
    - バリデーション: `limit` が 100 超 → 400
    - バリデーション: 不正な `cursor`（非base64url） → 400
    - バリデーション: 不正な `cursor`（非オブジェクトJSON） → 400
    - バリデーション: 不正な `status` → 400
    - 異常系: DynamoDB クエリ失敗 → 500
  - [x] 8.5.2. `routes/jobs.ts` に `GET /` ルートを実装してテストを通す
    - GSI `status-created_at-index` で `QueryCommand`
    - `cursor` は Base64url エンコードした `LastEvaluatedKey`（オブジェクト型検証付き）
    - `limit` のデフォルトは 20、上限 100
    - `count` は `items.length` を使用（FilterExpression 追加時にも正確）
    - ステータスバリデーション（PENDING/PROCESSING/COMPLETED/FAILED/CANCELLED）
    - `ScanIndexForward: false`（新しい順）

- [x] 8.6. `DELETE /jobs/:jobId` のテストと実装（TDD）（P0、8.2 完了後）
  - [x] 8.6.1. テストを作成する（8テストケース）
    ...
    - 異常系: UUID 形式でない jobId → 400
    - 異常系: DynamoDB UpdateCommand 失敗（非 ConditionalCheck）→ 500
  - [x] 8.6.2. `routes/jobs.ts` に `DELETE /:jobId` ルートを実装してテストを通す
    - `UpdateCommand` で `ConditionExpression="#s = :pending"` 付きで `CANCELLED` に遷移
    - `ConditionalCheckFailedException` → `GetItem`（`ConsistentRead: true`）でレコード存在確認、なければ 404、あれば 409
    - S3 `deleteObject` をベストエフォートで実行（try-catch で失敗は無視）
    - レスポンスは DB の `ReturnValues: "ALL_NEW"` から取得

- [x] 8.7. エントリポイントとエラーハンドリングを統合する（P0、8.3-8.6 完了後）
  - [x] 8.7.1. `index.ts` を実装
    - Hono app を作成し `/jobs` ルートをマウント
    - `handle(app)` で Lambda ハンドラをエクスポート（`hono/aws-lambda` アダプター使用）
    - グローバルエラーハンドラ（`app.onError`）で `ValidationError` → 400、`NotFoundError` → 404、`ConflictError` → 409、その他 → 500
  - [x] 8.7.2. 全テスト実行: 全 59 テスト PASS、型チェック OK

- [x] 8.8. Biome で lint / format を実行し修正する（P0、8.7 完了後）
  - [x] 8.8.1. `npx @biomejs/biome check --write lambda/api/` で自動修正（import ソート、フォーマット等 5 ファイル修正）
  - [x] 8.8.2. `biome-ignore` コメント追加（テスト用 `any` 型、セキュリティ用制御文字 regex）
  - [x] 8.8.3. `npx @biomejs/biome check lambda/api/` でエラーゼロを確認
...
- [x] 9.1. `lib/api-stack.ts` の基本構成を作成する（P0）
  - [x] 9.1.1. CDK スナップショットテストを先に作成する（TDD: `test/api-stack.test.ts`）
    - `ApiStackProps` として `bucket: Bucket` と `statusTable: Table` を受け取ること
    - `NodejsFunction` が作成されること（ランタイム Node.js 24.x、タイムアウト 29s、メモリ 256MB）
    - Lambda 環境変数に `STATUS_TABLE_NAME`, `BUCKET_NAME` が設定されていること
    - `LambdaRestApi` が作成されること（`endpointTypes: REGIONAL`）
    - 全 6 テスト PASS
  - [x] 9.1.2. `lib/api-stack.ts` を実装してテストを通す
    - `ApiStackProps` インターフェース定義（`bucket: Bucket`, `statusTable: Table`）
    - `NodejsFunction` を作成（`entry: lambda/api/index.ts`, `runtime: NODEJS_24_X`, `handler: handler`, `bundling: { minify: true }`, `memorySize: 256`, `timeout: 29s`）
...
- [x] 10.1. デプロイする（P0、フェーズ 9 完了後）
  - [x] 10.1.1. `npx cdk deploy --all` で全スタックをデプロイ
    - 初回デプロイ時に CloudWatch Logs ロール未設定エラー → CfnAccount + IAM ロールを追加して解決
  - [x] 10.1.2. CloudFormation Output から以下を記録:
    - `ApiStack.DistributionDomainName` = `d3e7amrzxi83p1.cloudfront.net`
    - `ApiStack.ApiKeyId` = `igtb08c7p2`
  - [x] 10.1.3. API Key の値を取得: `WxKyruWK...`

- [x] 10.2. 結合テストスクリプトを作成する（P0、10.1 完了後）
  - [x] 10.2.1. `scripts/test-api.py` を作成。CloudFormation Output から自動で URL / API Key を取得する構成
  - [x] 10.2.2. テスト A — POST /jobs: ジョブ作成 → PASS
    - 201 返却、`jobId`, `uploadUrl` 含む、presigned URL への PUT アップロード成功
  - [x] 10.2.3. テスト B — GET /jobs/:jobId: ステータス確認 → PASS
    - 作成直後に GET → `status: "PENDING"`
  - [x] 10.2.4. テスト C — GET /jobs: 一覧取得 → PASS
    - `?status=PENDING` で一覧取得、`items` に結果あり
    - `?status=PENDING&limit=1` → `cursor` 返却、次ページ取得成功
  - [x] 10.2.5. テスト D — DELETE /jobs/:jobId: キャンセル → PASS
    - DELETE → 200 + `status: "CANCELLED"`、再 GET で確認、再 DELETE → 409
  - [x] 10.2.6. テスト E — バリデーション → PASS
    - filename なし → 400、`.txt` → 400、存在しない jobId → 404
  - [x] 10.2.7. テスト F — API Key なしでアクセス → 403 → PASS

- [x] 10.3. E2E フローテスト（P0、10.2 完了後）
  - [x] 10.3.1. テスト G — OCR 完了まで通しテスト（エンドポイント起動済みの場合） → PASS
    - POST /jobs → uploadUrl に PDF を PUT → ポーリングで COMPLETED まで待機 → `resultUrl` で結果 JSON を取得 → JSON の内容を検証
    - `result_key` → `output_key` 属性名不一致バグを修正（Processor は `output_key` で DynamoDB に保存、API は `result_key` を参照していた）
    - OCR 結果: "Document 2: Receipt / Store: Test Shop / Total: 5000 JPY" を正しく認識、processingTimeMs: 1023
  - [x] 10.3.2. テスト H — CloudFront 経由でない直接アクセスの拒否確認 → PASS
    - API Gateway の URL に直接リクエスト → 403 Forbidden

- [x] 10.4. テスト結果をまとめる（P0、10.3 完了後）
  - [x] 10.4.1. 全テスト結果を確認し、問題があれば修正
    - テスト C のレスポンスキー名修正（`jobs` → `items`）、`status` 必須パラメータ対応
  - [x] 10.4.2. テスト結果のサマリ: テスト A-H 全 PASS
