...
3.5. (Office 形式が含まれる場合のみ) ``office_converter.convert_office_files``
   で `.pptx` / `.docx` / `.xlsx` を PDF に変換し、変換失敗を
   ``process_log.jsonl`` に ``error_category=CONVERSION_FAILED`` で追記
...
def _append_conversion_failures_to_log(
    log_path: Path, failed: Iterable[ConvertFailure]
) -> None:
    """Office → PDF 変換失敗を ``process_log.jsonl`` に直接追記する。

    yomitoku-client (``run_async_batch``) が同じファイルに書き込む前にこの追記が走るため、
    `error_category="CONVERSION_FAILED"` と OCR 失敗 (``"OCR_FAILED"`` に正規化される
    ``error_category=None``) を行ごとに区別できる。

    Schema (``process_log_reader.ProcessLogEntry`` と整合):
        - ``timestamp``: ISO8601 (UTC, ``+00:00``)
        - ``file_path``: 元 Office ファイルのローカル絶対パス (削除後でも DDB FILE PK
          特定に使えるよう原本パスをそのまま記録)
        - ``filename``: ``Path.name``
        - ``success``: 常に ``False``
        - ``error``: ``ConvertFailure.detail`` (人間可読の失敗詳細)
        - ``error_category``: ``"CONVERSION_FAILED"`` (固定)

    log_path の親ディレクトリが未作成のケースに備えて ``mkdir(parents=True, exist_ok=True)``
    を行う。1 ファイル 1 行 (JSON Lines) で append-only。
    """
    ...
    with log_path.open("a", encoding="utf-8") as fp:
        for failure in failed_list:
            record = {
                ...
                "file_path": str(failure.original_path),
                "filename": failure.original_path.name,
                "success": False,
...
def run(settings: BatchRunnerSettings) -> int:
    ...
    try:
        ...
        if office_files_present:
            ...
            logger.info(
                "convert_office_files complete",
                extra={
                    "batch_job_id": settings.batch_job_id,
                    "converted_succeeded": len(convert_result.succeeded),
