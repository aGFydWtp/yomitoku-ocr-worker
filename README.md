...
| パラメータ | 必須 | 説明 |
|-----------|------|------|
| `batchLabel` | No | バッチ一覧で識別するための任意の表示用ラベル。S3 キーや DynamoDB PK/SK には含まれない。英数字・日本語・ハイフン・アンダースコア・ドット・スラッシュのみ、512 バイト以内。省略時はレスポンスで `null` |
...
終端状態 (`COMPLETED` / `PARTIAL` / `FAILED`) のバッチから、失敗したファイルだけを新しい子バッチに複製して再実行します。元バッチの `batchLabel` と入力 S3 オブジェクトをそのまま継承し、`parentBatchJobId` で追跡できます。
...
| ステータスコード | 説明 |
|----------------|------|
| 400 | バリデーションエラー（不正な batchLabel、100 ファイル超過、非 PDF など） |
...
lambda/
  ...
  api/                        REST API (Hono + Zod-OpenAPI, TypeScript) ← pnpm workspace
    ...
    lib/
      ...
      validate.ts             batchLabel / SFN ARN バリデーション
