...
npm run lint:legacy    # 旧 API 参照が残っていないか検査（CI ガード）
...
lib/
  ...
  processing-stack.ts         S3 (batches/*) / ControlTable / BatchTable (+ GSI1/GSI2)
  orchestration-stack.ts      EndpointControl Step Functions / EventBridge / heartbeat
  batch-execution-stack.ts    BatchExecution Step Functions / ECS Fargate Task
  api-stack.ts                API Gateway / CloudFront / API Lambda (batch-first IAM)
  monitoring-stack.ts         CloudWatch Alarms (FilesFailed, BatchDuration) / SNS
pnpm-workspace.yaml           ワークスペース定義
lambda/
  endpoint-control/           SageMaker エンドポイント制御 (Python)
  batch-runner/               BatchRunnerTask 本体（yomitoku-client 呼び出し + S3/DDB 同期）
  api/                        REST API (Hono, TypeScript) ← pnpm workspace
    index.ts                  /batches・/status・/up ルート + Swagger UI
    ...
    lib/
      ...
      batch-presign.ts        S3 presigned URL 発行
      validate.ts             basePath / cursor バリデーション + SFN ARN 検証
      errors.ts               エラークラス + ハンドラ
      s3.ts                   S3 操作 (presigned URL, 削除, HEAD)
...
docs/archive/                 旧設計資料（`/jobs` ベース）のアーカイブ
```
