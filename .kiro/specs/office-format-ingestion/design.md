...
- **`apply_process_log` の DDB FILE PK 解決 (Bug 001 fix / ultrareview 後追加)**: 変換成功 PPTX (例: `deck.pptx`) は yomitoku-client が `process_log.jsonl` に変換後 PDF 名 (`deck.pdf`) を書き出すため、`apply_process_log` がそのまま filename ベースで PK を組むと **API が seed した原本 PK (`FILE#.../deck.pptx`) と一致せず**、`deck.pdf` 名で phantom FILE 行が upsert され原本行が PENDING のまま残置されていた。本 spec では `main.py` が `convert_office_files` の `succeeded` から `{converted_pdf_basename: original_filename}` マップ (例: `{"deck.pdf": "deck.pptx"}`) を組み立て `apply_process_log(..., converted_filename_map=...)` に渡す。`apply_process_log` 側は filename を PK 解決時にこのマップで原本名へ書き戻す。`resultKey` (= `batches/{id}/output/<stem>.json`) は変換後 stem ベースのまま (実 S3 出力と整合) で書き換えない。これにより `result-filename-extension-preservation` spec の **PK 半分** が前倒しで解消される (JSON 命名 (`{stem}.json` → `{full_filename}.json`) の半分は引き続き別 spec で扱う)。
