...
2. **TargetTracking ポリシーを CloudWatch Math 式に変更** (`lib/sagemaker-stack.ts:407-428`):
   ...
   - 式 (CDK で `CfnScalingPolicy.TargetTrackingMetricDataQuery` 配列として表現):
     ```
    m1 = ApproximateBacklogSize  (Average, AWS/SageMaker, dim: EndpointName)
    m2 = InflightInvocations     (Sum, Yomitoku/AsyncEndpoint, dim: EndpointName)
    e1 = FILL(m1, 0) + IF(FILL(m2, 0) > 0, 5, 0)   // ReturnData=true
     ```
