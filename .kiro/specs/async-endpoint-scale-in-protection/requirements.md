...
1. The 本機能 shall ターゲット追跡ポリシーのカスタムメトリクスを「`AWS/SageMaker::ApproximateBacklogSize` (Sum, 60s) と `Yomitoku/AsyncEndpoint::InflightInvocations` (Sum, 60s) の合算を返す CloudWatch metric math 式 (single time series, ReturnData=true 1 つ)」に置き換える。
